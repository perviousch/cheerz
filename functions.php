...
function handleDeleteEntry($conn) {
    // Ensure the delete_date is present in the POST data
    if (!isset($_POST['delete_date'])) {
        echo "Error: Date not provided for deletion.";
        return;
    }
    
    // Get the registration_date from POST data
    $registration_date = $_POST['delete_date'];
    
    // Debugging: Ensure registration_date is correctly retrieved
    if (!$registration_date) {
        echo "Error: No date provided for deletion.";
        return;
    }
    
    // Prepare and execute the SQL delete statement
    $stmt = $conn->prepare("DELETE FROM loyalty_registration WHERE registration_date = ?");
    $stmt->execute([$registration_date]);
